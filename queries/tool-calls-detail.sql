...
-- Heuristic: result string contains "error" or is_error:true.
WITH calls AS (
...
SELECT
  ...
  count(*) FILTER (WHERE r.out_str ILIKE '%"is_error":true%'
                      OR r.out_str ILIKE '%"error"%')       AS errors,
  round(100.0 * count(*) FILTER (WHERE r.out_str ILIKE '%"is_error":true%'
                                    OR r.out_str ILIKE '%"error"%')
